			...
			db.get(trip_id, function(err, body){
				if(err){
					...
					if(body.routes && body.routes.length > 0){
						...
						deferred.resolve({
							trip_id: trip_id, 
							start_longitude: (route0.matched_longitude || route0.lng || route0.longitude) - 0, 
							start_latitude: (route0.matched_latitude || route0.lat || route0.latitude) - 0,
							end_longitude: (routeZ.matched_longitude || routeZ.lng || routeZ.longitude) - 0,
							end_latitude: (routeZ.matched_latitude || routeZ.lat || routeZ.latitude) - 0, 
							start_time: moment(route0.ts || route0.timestamp).valueOf(),
							end_time: moment(routeZ.ts || routeZ.timestamp).valueOf()
						});
					}else{
