...
var DEFAULT_EVENT_END_TIME = process.env.DEFAULT_EVENT_END_TIME || 30;
...
_.extend(vehicleDataHub, {
	...
	createEvent: function (event, op) {
		var node = this.vdhConfig;
		if (!event.end_time) {
			event.end_time = moment(event.start_time || event.event_time).add(DEFAULT_EVENT_END_TIME, "days").toISOString();
		}
		var options = this._makeRequestOptions(node, {
			method: 'POST',
